        ...
        if(!empty($email))
        {
            ...
            else 
            {
                if($user = $userObj->emailExist($email))
                {
                    ...
                    $message = "{$user->firstName}, Someone requested for new password, if you didn't requested new password then leave this email, however if you did then here's your reset link <a href='http://localhost:81/login-system/account/recover/{$link}'>Reset Password</a>";
                    $subject = "Reset password";
                    $verifyObj->sendToMail($user->email, $message, $subject);
                    $userObj->insert('recovery', array('user_id' => $user-> user_id, 'code' => $link));
                    $userObj->redirect('account/recover/?email=sent');
                } 
                else
                {
	...
	<div class="wrapper">
		...
		<div class="sign-in">
            ...
            if(isset($_GET['email']) || !empty($_GET['email'])){
                echo '<div class="success-message">An email has been sent to your email address with reset password link</div>';
            } 
            else
            {
