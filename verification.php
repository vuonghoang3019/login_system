<?php 
	...
	$verifyObj->authOnly();
		   ...
		   $message = "{$user->firstName}, Your account has been created, Visit this link to verify your account : <a href='http://localhost:81/login-system/verification/{$link}'>Verify Link</a>";
	   ...
	   if (isset($_POST['phone']))
	   {
			...
			if(!empty($number))
			{
				if(preg_match("/^([0-9]+)$/",$number))
				{
					...
					$verifyObj -> senToPhone($number,$message);
		...
		<div class="sign-up-inner">
			<div class="sign-up-div">
				...
				<?php if(isset($_GET['email'])): ?>
					<h4>An verification email has been sent to you account, check your email to verify your account</h4>
				<?php else: ?>
					<h3>Email Verification</h3>
					<form method="POST">
					<input type="email"  placeholder="<?php echo $user->email;?>" value="<?php echo $user->email;?>" />
					<button type="submit" class="suc" name="email">Send me verification email</button>
					</form>
				<?php endif ?>
				</fieldset>
				</div>
